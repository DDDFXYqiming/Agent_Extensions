...
本文件的相对路径是 `ppt-master/templates/layouts/重庆大学/design_spec.md`，属于 `AI_Projects` 的参考/工作流/模板/脚本/资产之一。
...
- 本节说明 `设计 功能` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
...
### 中文：Primary 颜色 Extracted from 标志

- 本节说明 `Primary 颜色 Extracted from 标志` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：Primary 颜色 Extracted from 标志
...
- 本节说明 `渐变 Scheme` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
...
- 本节说明 `Core 视觉 Elements` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：核心 Visual Elements

### 1. Diagonal Color Blocks (Mountain City Layers)

### 中文：Diagonal 颜色 Blocks Mountain City Layers

- 本节说明 `Diagonal 颜色 Blocks Mountain City Layers` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：Diagonal 颜色 Blocks Mountain City Layers
...
### 中文：Wave Patterns Two Rivers Imagery

- 本节说明 `Wave Patterns Two Rivers Imagery` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：2. Wave Patterns (Two Rivers Imagery)
...
### 中文：Light Dot Decorations City Lights

- 本节说明 `Light Dot Decorations City Lights` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：3. Light Dot Decorations (City Lights)
...
- Layout Structure**（以上为中文意译，具体细节以英文原文为准）。
- Upper-right area: Logo (using logo（以上为中文意译，具体细节以英文原文为准）。
- png)（以上为中文意译，具体细节以英文原文为准）。
- Center-left: Main title + subtitle（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Layout Structure**（以上为中文意译，具体细节以英文原文为准）。
- Full-screen deep blue background（以上为中文意译，具体细节以英文原文为准）。
- Upper-right: Diagonal light area (sky blue gradient)（以上为中文意译，具体细节以英文原文为准）。
- Left: Large chapter number (semi-transparent)（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Layout Structure**（以上为中文意译，具体细节以英文原文为准）。
- Top: Diagonal blue accent strip (approx（以上为中文意译，具体细节以英文原文为准）。
- 80px height, higher on left, lower on right)（以上为中文意译，具体细节以英文原文为准）。
- On the accent strip: Page title + Logo（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Layout Structure**（以上为中文意译，具体细节以英文原文为准）。
- Center: Large-sized Logo（以上为中文意译，具体细节以英文原文为准）。
- Below logo: Thank-you message（以上为中文意译，具体细节以英文原文为准）。
- Bottom diagonal blue area: Contact information（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Layout Structure**（以上为中文意译，具体细节以英文原文为准）。
- Top diagonal accent strip + title（以上为中文意译，具体细节以英文原文为准）。
- Left: Large numeric indices (vertically arranged, with gold accents)（以上为中文意译，具体细节以英文原文为准）。
- Right: TOC item text（以上为中文意译，具体细节以英文原文为准）。
- Right: TOC item text
...
- 本节说明 `VII 标志 使用方法 指南` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：VII 标志 使用方法 指南
