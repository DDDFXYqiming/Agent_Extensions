...
本文件的相对路径是 `ppt-master/templates/layouts/china_telecom_template/design_spec.md`，属于 `AI_Projects` 的参考/工作流/模板/脚本/资产之一。
...
- 本节说明 `Secondary 颜色` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：Secondary 颜色
...
- 本节说明 `Structural 规则` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：Structural 规则
...
- Top-left fixed logo（以上为中文意译，具体细节以英文原文为准）。
- Left-aligned title cluster with red accent rule（以上为中文意译，具体细节以英文原文为准）。
- Right-side visual card containing slogan and skyline imagery（以上为中文意译，具体细节以英文原文为准）。
- Bottom full-width ribbon background（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Red rounded title capsule + gray structural lane（以上为中文意译，具体细节以英文原文为准）。
- Top-right compact logo for page-level brand anchoring（以上为中文意译，具体细节以英文原文为准）。
- Left visual card with restrained brand imagery（以上为中文意译，具体细节以英文原文为准）。
- Right text list area for up to 4 major sections（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Clean section-divider page without the content-page header ribbon（以上为中文意译，具体细节以英文原文为准）。
- Top-right compact logo anchored away from the title area（以上为中文意译，具体细节以英文原文为准）。
- Large chapter number and title in the left safe zone（以上为中文意译，具体细节以英文原文为准）。
- Right-side visual card with fixed imagery and no duplicated large logo（以上为中文意译，具体细节以英文原文为准）。
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Red section tab at top-left, gray lane at top-right（以上为中文意译，具体细节以英文原文为准）。
- Top-right compact logo for page-level brand anchoring（以上为中文意译，具体细节以英文原文为准）。
- Open-canvas content area for flexible charts, tables, and mixed layouts（以上为中文意译，具体细节以英文原文为准）。
- Only keep lightweight corner / footer-level brand control（以上为中文意译，具体细节以英文原文为准）。
...
- Left closing statement block（以上为中文意译，具体细节以英文原文为准）。
- Right closing visual card with restrained skyline / slogan composition（以上为中文意译，具体细节以英文原文为准）。
- Full-width footer ribbon（以上为中文意译，具体细节以英文原文为准）。
...
- 本节说明 `XI 使用方法 指南` 相关的任务目标、输入、输出、关键约束和验收标准。
- 优先用中文向用户解释本节做什么、怎么用、失败时怎么办；下方保留的英文细则、参数、代码和命令作为机器可读规范。
- 执行前先核对任务前置条件：所需文件、所需依赖、所需环境变量、所需权限是否齐备。
- 执行后必须验证：文件是否存在、命令是否成功、产物是否完整、是否引入敏感信息。
- 如果本节是参考性章节而不是操作步骤，重点是给读者一个中文索引和阅读顺序，而不是直接产生新文件。
### 中文：中文标题：XI 使用方法 Guide
