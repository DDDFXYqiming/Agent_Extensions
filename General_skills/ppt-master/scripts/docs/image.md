...
## 中文主体补充：AI_Projects / image.md

本文件属于公开技能 `AI_Projects`。本节为中文主体补充说明，目标是让中文读者在不依赖英文背景的情况下，理解本文件的作用、阅读路径、关键约束和验收标准。
本文件的相对路径是 `ppt-master/scripts/docs/image.md`，属于 `AI_Projects` 的参考/工作流/模板/脚本/资产之一。
...
## 中文长概览：skill / image.md

本文件属于公开技能 `skill` 下的 `image.md` 文档，目标是把幻灯片、模板、图表、配图、动画或脚本相关的全部规格统一收纳，便于复用与扩展。
...
## 中文主体说明：Hermes_Agent_Resources 文档：image
...
- Unified image generation entry point（以上为中文意译，具体细节以英文原文为准）。
```bash
...
### 中文：analyze_images py

- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Analyze images in a project directory before writing the design spec or composing slide layouts（以上为中文意译，具体细节以英文原文为准）。
```bash
...
- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Zero-config web image search across openly-licensed providers（以上为中文意译，具体细节以英文原文为准）。
- Sister tool to `image_gen（以上为中文意译，具体细节以英文原文为准）。
- py` — used when the resource list row has `Acquire Via: web`（以上为中文意译，具体细节以英文原文为准）。
```bash
...
### 中文：gemini_watermark_remover py

- 本节对应原始英文段落。中文部分负责解释目标、流程、风险与验收，不替代下方英文细则。
- Remove Gemini watermark assets after manual download（以上为中文意译，具体细节以英文原文为准）。
```bash
